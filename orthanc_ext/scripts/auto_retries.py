...
def calculate_delay(job, first_retry=ONE_MINUTE):
    elapsed = parse_time(
        job['CompletionTime']) - parse_time(job['CreationTime'])
...
def handle_failed_forwarding_job(
        first_retry=ONE_MINUTE,
        job_types=RETRYABLE_JOBTYPES):
    ...
    def handle_failed_forwarding_job(event, client):
        ...
        job = response.json()
        if (job_type := job['Type']) not in job_types:
