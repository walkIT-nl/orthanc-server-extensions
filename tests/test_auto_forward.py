...
from orthanc_ext.scripts.auto_forward import (
    forward_dicom, DicomReceivedMatcher)
...
def test_autoforward_on_match_shall_start_start_modality_store(caplog):
    store = respx.post(
        '/modalities/pacs/store').respond(200, text='study-uuid')
    register_and_trigger_handler(
        [DicomReceivedMatcher(lambda uid, _: True, lambda uid, _: 'pacs')])
    assert store.called
...
def test_autoforward_on_multiple_matches_shall_start_start_modality_store(caplog):
    instances = respx.get(
        '/series/study-uuid/instances').respond(
            200, json=[
    ...
    origin = respx.get(
        ...
        '/metadata/Origin').respond(
            200, text='Plugins')
    pacs1 = respx.post(
        '/modalities/pacs1/store').respond(
            200, text='study-uuid')
    pacs2 = respx.post(
        '/modalities/pacs2/store').respond(
            200, text='study-uuid')
